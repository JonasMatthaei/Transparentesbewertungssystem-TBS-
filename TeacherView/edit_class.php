...
if(isset($_GET["preload"])){
    $all_names = preload();
    $old_color = getPreDataColor();
    $old_name = getPreDataName();
    
    setcookie("add_schueler", $_COOKIE["add_schueler"] . $all_names);
    setcookie("color", $old_color);
    setcookie("name", $old_name);

    header('Location: edit_class.php?kurs=' . $_GET["kurs"]);
    exit();
}
...
if(isset($_GET["delkurs"])){
    if(!empty($_GET["delkurs"])){
        if($_GET["delkurs"] == "false"){
            echo '<script> var check = confirm("Wollen Sie diesen Kurs Löschen?"); 
                    if (check == true) {
                        window.location = "edit_class.php?kurs=' . $_GET["kurs"] . '&delkurs=true";
                    }</script>';
        }
        else{
            delKurs();
    ...
    <div class="mdl-layout mdl-js-layout mdl-layout--fixed-header">
        ...
        <main class="mdl-layout__content">
            <div class="page-content">
                ...
                $old_name = getOldKursName($_GET["kurs"]);
                echo '<h3 style="text-align:center;">Bearbeitung des Kurses: ' . $old_name . '</h3>';
                ...
                <div class="main-section">
                    ...
                    <div class="dropdown-menu">
                        ...
                        <div class="Hinzufügen-Button">
                            <a class="mdl-button mdl-js-button mdl-button--raised mdl-button--colored" href= "edit_class.php?kurs=<?php echo $_GET["kurs"]?>&add=schueler">Add</a>
                        </div>
                    ...
                    <div class="dropdown-menu">
                        ...
                        <div class="Hinzufügen-Button">
                            <a class="mdl-button mdl-js-button mdl-button--raised mdl-button--colored" href= "edit_class.php?kurs=<?php echo $_GET["kurs"]?>&add=klasse">Add</a>
                        </div>
                    ...
                    <p><a class="mdl-button mdl-js-button mdl-button--raised mdl-button--colored" href="edit_class.php?kurs=<?php echo $_GET["kurs"]?>&delkurs=false">Kurs Löschen</a></p>
                    <p><a class="btn-sp-ab" href="edit_class.php?kurs=<?php echo $_GET["kurs"]?>&safe=true">Speichern</a>  <a class="btn-sp-ab" href="edit_class.php?kurs=<?php echo $_GET["kurs"]?>&safe=false">Abbrechen</a></p>
            ...
            try{
                ...
                if(count($split) == 2){
                    ...
                    $tmp_data = $tmp_data . '<div class="show-student">
                            <p>' . $name . '</p>
                            <a class="remove_button" href="edit_class.php?kurs=' . $_GET["kurs"] . '&del=' . $name . '_' . $id . '">
                                Entfernen
                            </a>
                        </div>';
                }
...
function getJahrgang(){
	...
	while ($row = mysqli_fetch_array($result)){
        $tmp_data = $tmp_data . '<a href="edit_class.php?kurs=' . $_GET["kurs"] . '&jahrgang=' . $row[0] . '">' . $row[0] . '</a>';
    }
...
function getKlassen(){
	...
	while ($row = mysqli_fetch_array($result)){
        $tmp_data = $tmp_data . '<a href="edit_class.php?kurs=' . $_GET["kurs"] . '&klasse=' . $row[0] . '">' . $row[0] . '</a>';
    }
	...
	while ($row = mysqli_fetch_array($result)){
        if(!(empty($_COOKIE["add_schueler"]))){
            if(stripos($_COOKIE["add_schueler"], $row[2] . "." . $row[1] . "_" . $row[0], 0) == FALSE){
                $tmp_data = $tmp_data . '<a href=edit_class.php?kurs=' . $_GET["kurs"] . '&schueler=' . $row[2] . '.' . $row[1] . '_' . $row[0] . '">' . $row[2] . '.' . $row[1] . '</a>';
            }
        }
        else{
            $tmp_data = $tmp_data . '<a href="edit_class.php?kurs=' . $_GET["kurs"] . '&schueler=' . $row[2] . '.' . $row[1] . '_' . $row[0] . '">' . $row[2] . '.' . $row[1] . '</a>';
...
function getAllSchuelerIDs(){
    require "../includes/db.php";

    $stmt = mysqli_stmt_init($conn);
    $sql = 'select * from teilnehmende where Kurs_ID="' . $_GET["kurs"] . '";';
...
function preload(){
    ...
    $ID_list = getAllSchuelerIDs();
    $tmp_data = "";
    
    foreach($ID_list as $schueler_id){
        ...
        if ($row = mysqli_fetch_assoc($result)){
            $tmp_data = $tmp_data . "-" . $row["Nachname"] . "." . $row["Name"] . "_" . $row["Schueler_ID"];
        }
    }
...
function getPreDataName(){
    require "../includes/db.php";

    $stmt = mysqli_stmt_init($conn);
    $sql = 'select * from kurs where Kurs_ID="' . $_GET["kurs"] . '";';
...
function getOldKursName($tmp_KursID){
    require "../includes/db.php";
...
function delKurs(){
    ...
    $sql = 'delete from kurs where Kurs_ID="' . $_GET["kurs"] . '";';
