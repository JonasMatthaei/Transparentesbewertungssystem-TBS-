                ...
                <div class=" header">
                    <div class="mdl-card__title" style=<?php echo '"background-color: ' . getFarbe($_GET["kurs"]) . '"'; ?>>
                        ...
                        <ul class="mdl-menu mdl-menu--bottom-left mdl-js-menu mdl-js-ripple-effect"for="demo-menu-lower-left">      
                            <li class="mdl-menu__item"><a href="student_class_overview.php">Kurse</a> </li>
                            <?php
                            
                            echo '<li class="mdl-menu__item"><a href="line.php?kurs=' . $_GET["kurs"] . '">Visualisierung</a></li>'; 
                ...
                <table class="mdl-data-table  mdl-shadow--2dp attendance-table">
                    <thead>
                      <tr>
                        ...
                        $data = showLastDates();
                        if($data != ""){
                            echo '<th class="thead-text" >Du</th>';
                            echo $data;
                        }
                    ...
                    $data = showSchueler();
                    if($data == "error1"){
                        echo '</table><p style="color:red;text-align:center;">Es hat noch keine Unterrichts stunde gegeben!</p><table>';
                    }
                    else if($data == "error2"){
                        echo '</tr></tbody></table><p style="color:red;text-align:center;">Es hat noch keine Bewertung stattgefunden!</p><table>';
                    }
                    else{
                        echo $data;
                    }
...
function getName($tmp_kursID){
	...
	mysqli_stmt_prepare($stmt, $sql);
	mysqli_stmt_bind_param($stmt, "s", $tmp_kursID);
	mysqli_stmt_execute($stmt);
...
function showSchueler(){
    ...
    $tmp_data = $tmp_data . '<tbody><tr><td style="width:100px;text-align:center !important;" >' . $_SESSION["Name"] . '</td>';

    foreach($datums as $date){
        $anwesend = getAnwesend($_SESSION["ID"], $date);
        $status = "/";
        $class = "present";
        ...
        $tmp_data = $tmp_data . '<td style="text-align:center !important;"><a style="width:100px" class="' . $class . '">' . $status . '</a></td>';

        $count++;
    }

    if($count == 0) return 'error1';
    else if(count($datums) == 0) return $tmp_data . 'error2';
